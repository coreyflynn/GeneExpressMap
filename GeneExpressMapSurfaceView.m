function varargout = GeneExpressMapSurfaceView(varargin)
% GENEEXPRESSMAPSURFACEVIEW M-file for GeneExpressMapSurfaceView.fig
%      GENEEXPRESSMAPSURFACEVIEW, by itself, creates a new GENEEXPRESSMAPSURFACEVIEW or raises the existing
%      singleton*.
%
%      H = GENEEXPRESSMAPSURFACEVIEW returns the handle to a new GENEEXPRESSMAPSURFACEVIEW or the handle to
%      the existing singleton*.
%
%      GENEEXPRESSMAPSURFACEVIEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GENEEXPRESSMAPSURFACEVIEW.M with the given input arguments.
%
%      GENEEXPRESSMAPSURFACEVIEW('Property','Value',...) creates a new GENEEXPRESSMAPSURFACEVIEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GeneExpressMapSurfaceView_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GeneExpressMapSurfaceView_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GeneExpressMapSurfaceView

% Last Modified by GUIDE v2.5 09-Mar-2011 12:36:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GeneExpressMapSurfaceView_OpeningFcn, ...
                   'gui_OutputFcn',  @GeneExpressMapSurfaceView_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GeneExpressMapSurfaceView is made visible.
function GeneExpressMapSurfaceView_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to GeneExpressMapSurfaceView (see VARARGIN)

% Choose default command line output for GeneExpressMapSurfaceView
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GeneExpressMapSurfaceView wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GeneExpressMapSurfaceView_OutputFcn(hObject, eventdata, handles) 
